...
# /home/ang037@AD.UCSD.EDU/ROADIES-with-TWILIGHT/TWILIGHT/bin/twilight -a $ref_msa -i $seqFile -o $workDir/iter0_msa.aln -C $threads --match 40 --mismatch -7 --transition 17 --gap-open -140 --gap-extend -10 --overwrite
...
# /home/ang037@AD.UCSD.EDU/ROADIES-with-TWILIGHT/TWILIGHT/bin/twilight -t $workDir/iter0_tree_output/epa_result.newick -i $workDir/iter1_input.fa -o $output_msa -C $threads --match 40 --mismatch -7 --transition 17 --gap-open -140 --gap-extend -10 --overwrite

# mkdir $workDir/iter1_tree_output

# raxml-ng --msa $output_msa --model GTR+G+F --threads auto{{$threads}} --blopt nr_safe --tree-constraint $ref_gene_tree --prefix $workDir/iter1_tree_output/gene_tree --redo
